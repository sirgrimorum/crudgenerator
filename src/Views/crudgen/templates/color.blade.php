...
<div class="form-group row {{$config['class_formgroup']}} {{ $extraClassDiv }}" data-tipo='contenedor-campo' data-campo='{{$tabla . '_' . $extraId}}'>
    ...
    <div class="{{ $config['class_divinput'] }}" id="contenedor_color_{{ $tabla . "_" . $extraId }}">
...
<script>
    ...
    function {{ $tabla . "_" . $extraId }}Loader(){
        if (!{{ $tabla . "_" . $extraId }}Ejecutado){
            $('#contenedor_color_{{ $tabla . "_" . $extraId }}').colorpicker({
                popover: false,
                inline: true,
                format: {{ $format }},
                fallbackColor: "{{ $default }}",
                container: '#contenedor_color_{{ $tabla . "_" . $extraId }}'
            });
        }
    ...
    {{ $nameScriptLoader }}('colorpicker_min_js',"{{ $tabla . "_" . $extraId }}Loader();");
</script>
<?php
if ($js_section != "") {
    ?>

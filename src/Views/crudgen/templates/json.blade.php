...
<div class="form-group row {{$config['class_formgroup']}}" data-tipo='contenedor-campo' data-campo='{{$tabla . '_' . $extraId}}'>
    ...
    <div class="{{ $config['class_divinput'] }}">
        ...
        @endif
        <button class="btn btn-secondary mt-2" role="button" type="button" onclick="prettyPrint('{{$tabla . '_' . $extraId,$readonly}}')">{{trans("crudgenerator::admin.layout.labels.pretty_print")}}</button>
    </div>
</div>
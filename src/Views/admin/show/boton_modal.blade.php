    ...
    if (is_array($botones)) {
        ...
        foreach ($botones as $boton) {
            if (strpos($boton, "<") === false) {
                echo "<button onclick='$(\"#{$modeloUCF}_show_modal\").modal(\"show\");' class='" . $config['class_button'] . "'>$boton</button>";
            } else {
                echo $boton;
            }
        }
        ...
        if (strpos($botones, "<") === false) {
            echo "<button onclick='$(\"#{$modeloUCF}_show_modal\").modal(\"show\");' class='" . $config['class_button'] . "' >$botones</button>";
        } else {
            echo $botones;
        }
    }
...
@endif
<!-- Modal for show {{ $modeloUCF }} -->
@include('sirgrimorum::admin.show.modal', ["modelo" => $modeloUCF,"base_url" => $base_url, "plural" =>$plural,"config"

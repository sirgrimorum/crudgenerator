    ...
    if (is_array($botones)) {
        ...
        foreach ($botones as $boton) {
            if (strpos($boton, "<") === false) {
                echo "<button onclick='$(\"#{$modeloUCF}_edit_modal\").modal(\"show\");' class='" . $config['class_button'] . "'>$boton</button>";
            } else {
                echo $boton;
            }
        }
        ...
        if (strpos($botones, "<") === false) {
            echo "<button onclick='$(\"#{$modeloUCF}_edit_modal\").modal(\"show\");' class='" . $config['class_button'] . "' >$botones</button>";
        } else {
            echo $botones;
        }
    }
...
@endif
<!-- Modal for edit {{ $modeloUCF }} -->
@if($siOld)
<!-- Modal for old input -->
@include('sirgrimorum::admin.edit.modal', ["modelo" => $modeloUCF,"base_url" => $base_url, "plural" =>$plural,"config"
=> $config,"registro"=>old("_registro")])
@else
@include('sirgrimorum::admin.edit.modal', ["modelo" => $modeloUCF,"base_url" => $base_url, "plural" =>$plural,"config"
=> $config,"registro"=>$registro])
@endif
@if (config("sirgrimorum.crudgenerator.modal_section") != "")
@endpush
@endif
...
<script>
    $(document).ready(function() {
        $("#{{$modeloUCF}}_edit_modal").modal("show");
    });
</script>
@if ($js_section != "")
@endpush
